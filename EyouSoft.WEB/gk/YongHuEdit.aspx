<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="YongHuEdit.aspx.cs" Inherits="EyouSoft.WEB.gk.YongHuEdit" MasterPageFile="~/mp/Boxy.Master" %>
...
<asp:Content ContentPlaceHolderID="body" runat="server" ID="body1">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="mt10">
        ...
        <tr>
            ...
            <td>
                <input name="txtUsername" type="text" class="input-txt w400" id="txtUsername" runat="server" />
            </td>
        </tr>
        <tr>
            <td align="left">
            ...
            <td>
                <input name="txtMiMa" type="password" class="input-txt w400" id="txtMiMa" />
    ...
    <script type="text/javascript">
        var iPage = {
            ...
            yangZhengForm: function() {
                if ($("#<%=txt_gongsi_id.ClientID %>").length > 0 && $.trim($("#<%=txt_gongsi_id.ClientID %>").val()).length < 1) { alert("请选择<%=T %>"); return false; }
                if ($("#<%=txt_gongsi_name.ClientID %>").length > 0 && $.trim($("#<%=txt_gongsi_name.ClientID %>").val()).length < 1) { alert("请选择<%=T %>"); return false; }
            ...
            changeJueSe: function() {
                var _data = { txtGongSiId: $.trim($("#<%=txt_gongsi_id.ClientID %>").val()) };

                function _empty() {
                    $("#txtJueSe").empty();
                    $("#txtJueSe").append('<option value="">-请选择-</option>')
                }

                if (_data.txtGongSiId.length < 1) { _empty(); return; }

                function _cb(data) {
                    if (data.length == 0) { _empty(); return; }
                    var s = [];
                    s.push('<option value="">-请选择-</option>')
                    for (var i = 0; i < data.length; i++) {
                        s.push('<option value="' + data[i].JueSeId + '">' + data[i].Name + '</option>')
                    }
                    $("#txtJueSe").empty();
                    $("#txtJueSe").append(s.join(''));
                }

                $.ajax({ type: "get", url: window.location.href + "&doType=getgongsijuese", data: _data, cache: false, dataType: "json", async: true,
                    success: function(response) {
                        _cb(response);
                    }
                });
        ...
        $(document).ready(function() {
            ...
            if ($("#<%=txt_gongsi_id.ClientID %>").length > 0 && "<%=EditId %>".length > 0) { $("#<%=txt_gongsi_name.ClientID %>").prop("readonly", true).addClass("readonly"); }
            gongSiAutocomplete.init({ txt_id_id: "<%=txt_gongsi_id.ClientID %>", txt_name_id: "<%=txt_gongsi_name.ClientID %>", callback: function(data) { iPage.changeJueSe(); }, gslx: "<%=(int)T %>", pplx: 1 });
        });
    </script>
</asp:Content>